...
CLASS_UPDATE_ALIASES={'className':['serie','série','turma','classe'],'capacity':['limite','capacidade'],'students':['total alunos matri','total matriculado','matriculados','total alunos matriculados'],'new':['alunos novos','novos','novas matriculas','novas matrículas'],'renewed':['rematriculas','rematrículas'],'vacancies':['vagas','vagas disponiveis','vagas disponíveis']}
...
def _class_update_name(value):
    text=str(value or '').strip().upper().replace('�','º').replace('°','º').replace('ª','º')
    text=re.sub(r'(?<=\d)O(?=\s)', 'º', text);text=re.sub(r'\s+',' ',text)
    match=re.fullmatch(r'(\d+)º?\s*(EM|[A-D])',text)
    if match:return f'{int(match.group(1))}º {match.group(2)}'
    match=re.fullmatch(r'G\s*([2-5])\s*([A-C])',text)
    return f'G{match.group(1)} {match.group(2)}' if match else text

def class_update_records(name,content):
    """Extrai somente as seis colunas quantitativas do relatório de turmas."""
    ext=Path(name).suffix.lower()
    if ext=='.pdf':
        from pypdf import PdfReader
        reader=PdfReader(io.BytesIO(content))
        if len(reader.pages)>30:raise ValueError('Relatório de turmas excede 30 páginas.')
        text='\n'.join(page.extract_text(extraction_mode='layout') or '' for page in reader.pages)
        if not text.strip():raise ValueError('PDF sem texto selecionável. Forneça PDF textual ou XLSX.')
        records=[]
        for line in text.splitlines():
            match=re.match(r'^\s*((?:G\s*[2-5]|\d+[º°�O]?)\s*(?:EM|[A-D]))\s+(-?\d+)\s+(-?\d+)\s+(-?\d+)\s+(-?\d+)(?:\s+(-?\d+))?\s*$',line,re.I)
            if match:
                values=[int(value) for value in match.groups()[1:] if value is not None]
                if len(values)==5:records.append(dict(className=match.group(1),capacity=values[0],students=values[1],new=values[2],renewed=values[3],vacancies=values[4]))
        if not records:raise ValueError('Nenhuma linha de turma foi reconhecida no PDF.')
        return records,None
    if ext=='.xlsx':
        raw,sheet=read_rows(name,content,'class_update');records,_=mapped(raw,aliases=CLASS_UPDATE_ALIASES)
        return records,sheet
    raise ValueError('Formato permitido para atualização de turmas: PDF textual ou XLSX.')

def class_update_preview(name,content,state):
    """Compara um snapshot oficial com as turmas atuais, sem modificar o estado."""
    records,sheet=class_update_records(name,content);rooms={_class_update_name(room['name']):room for room in state.get('classes',[])};seen=set();parsed={};unrecognized=[];duplicates=[]
    for line,raw in enumerate(records,2):
        class_name=_class_update_name(raw.get('className'));room=rooms.get(class_name)
        try:
            values={key:int(decimal(raw.get(key))) for key in ['capacity','students','new','renewed','vacancies']}
            if any(values[key]<0 for key in ['capacity','students','new','renewed']):raise ValueError('Capacidade e matrículas não podem ser negativas.')
        except (ValueError,TypeError) as error:
            unrecognized.append({'line':line,'className':class_name,'reason':str(error)});continue
        if not room:unrecognized.append({'line':line,'className':class_name,'reason':'Turma não reconhecida com segurança.'});continue
        if room['id'] in seen:duplicates.append({'line':line,'className':class_name,'reason':'Turma repetida no documento.'});continue
        seen.add(room['id']);parsed[room['id']]=values
    rows=[]
    for room in state.get('classes',[]):
        current_new=int(room.get('opening',{}).get('new',0));current_re=int(room.get('opening',{}).get('re',0));current_total=int(room.get('students',current_new+current_re));current_capacity=int(room.get('capacity',0));current_vacancies=current_capacity-current_total;new=parsed.get(room['id'])
        if not new:
            rows.append({'classId':room['id'],'className':room['name'],'currentCapacity':current_capacity,'newCapacity':None,'currentRenewed':current_re,'newRenewed':None,'currentNew':current_new,'newNew':None,'currentTotal':current_total,'newTotal':None,'currentVacancies':current_vacancies,'newVacancies':None,'status':'missing','issues':['Turma ausente do documento.']});continue
        issues=[]
        if new['students']!=new['new']+new['renewed']:issues.append('Total do documento difere de novos + rematrículas.')
        if new['vacancies']!=new['capacity']-new['students']:issues.append('Vagas do documento diferem de capacidade - total.')
        changed=(current_capacity,current_re,current_new,current_total,current_vacancies)!=(new['capacity'],new['renewed'],new['new'],new['students'],new['vacancies'])
        status='divergence' if issues else 'above' if new['vacancies']<0 else 'update' if changed else 'unchanged'
        rows.append({'classId':room['id'],'className':room['name'],'currentCapacity':current_capacity,'newCapacity':new['capacity'],'currentRenewed':current_re,'newRenewed':new['renewed'],'currentNew':current_new,'newNew':new['new'],'currentTotal':current_total,'newTotal':new['students'],'currentVacancies':current_vacancies,'newVacancies':new['vacancies'],'status':status,'issues':issues})
    def totals(prefix):return {'capacity':sum((row[prefix+'Capacity'] or 0) for row in rows),'students':sum((row[prefix+'Total'] or 0) for row in rows),'new':sum((row[prefix+'New'] or 0) for row in rows),'renewed':sum((row[prefix+'Renewed'] or 0) for row in rows),'vacancies':sum((row[prefix+'Vacancies'] or 0) for row in rows)}
    digest=hashlib.sha256(content).hexdigest();already=any(item.get('kind')=='class_update' and item.get('fileHash')==digest for item in state.get('imports',[]));missing=[row['className'] for row in rows if row['status']=='missing'];divergent=[row['className'] for row in rows if row['status']=='divergence']
    return {'kind':'class_update','name':name,'sheet':sheet,'fileHash':digest,'rows':rows,'currentTotals':totals('current'),'newTotals':totals('new'),'missing':missing,'unrecognized':unrecognized,'duplicates':duplicates,'divergent':divergent,'duplicateFile':already,'canConfirm':len(rows)==41 and not missing and not unrecognized and not duplicates and not divergent and not already}

def apply_class_update_preview(state,preview,user):
    if not user.get('isAdmin'):raise PermissionError('Somente o Master pode confirmar atualização de turmas.')
    if not preview.get('canConfirm'):raise ValueError('A atualização possui pendências, divergências ou duplicidade e não pode ser confirmada.')
    updated=copy.deepcopy(state);by_id={row['classId']:row for row in preview['rows']};import_id=secrets.token_hex(12);applied_at=now();snapshots=[]
    for room in updated['classes']:
        row=by_id.get(room['id'])
        if not row or row['newTotal']!=row['newNew']+row['newRenewed'] or row['newVacancies']!=row['newCapacity']-row['newTotal']:raise ValueError('Validação por turma falhou durante a confirmação.')
        before={'capacity':room.get('capacity',0),'new':room.get('opening',{}).get('new',0),'renewed':room.get('opening',{}).get('re',0),'students':room.get('students',0),'vacancies':room.get('capacity',0)-room.get('students',0)};after={'capacity':row['newCapacity'],'new':row['newNew'],'renewed':row['newRenewed'],'students':row['newTotal'],'vacancies':row['newVacancies']};snapshots.append({'classId':room['id'],'className':room['name'],'before':before,'after':after})
        room['capacity']=after['capacity'];room['opening']={'new':after['new'],'re':after['renewed']};room['students']=after['students'];room['unclassified']=0;room['source']=preview['name'];room['quantitySnapshotId']=import_id
    for entry in updated.get('enrollments',{}).get('history',[]):entry.setdefault('supersededByClassUpdate',import_id)
    updated['enrollments']['new']=preview['newTotals']['new'];updated['enrollments']['re']=preview['newTotals']['renewed'];updated['enrollments']['unallocated']={'new':0,'re':0};updated.setdefault('imports',[]).append({'id':import_id,'kind':'class_update','name':preview['name'],'fileHash':preview['fileHash'],'date':applied_at,'userId':user['id'],'user':user['name'],'classesChanged':sum(row['status']!='unchanged' for row in preview['rows']),'previousTotals':preview['currentTotals'],'newTotals':preview['newTotals'],'snapshots':snapshots})
    return updated
...
def api_service(handler,user,path,p):
    store=handler.store
    if path=='/api/classes-update/preview':
        if not user.get('isAdmin'):raise PermissionError('Somente o Master pode importar atualização de turmas.')
        state,version=store.state();preview=class_update_preview(p['name'],base64.b64decode(p['content'],validate=True),state);token=secrets.token_urlsafe(28);preview['version']=version
    ...
    if path=='/api/classes-update/confirm':
        if not user.get('isAdmin'):raise PermissionError('Somente o Master pode confirmar atualização de turmas.')
        with store.lock,store.db() as db:
            ...
            if preview.get('kind')!='class_update':raise ValueError('Prévia incompatível com atualização de turmas.')
            if version!=row['version']:raise RuntimeError('Os dados mudaram desde a prévia. Gere uma nova prévia.')
            if p.get('confirmation')!='CONFIRMAR ATUALIZAÇÃO DE TURMAS':raise ValueError('Confirmação administrativa obrigatória.')
            updated=apply_class_update_preview(state,preview,user)
            from server import validate
            validate(updated,state);store.audit(db,user,'classes','import','quantitativos',preview['currentTotals'],preview['newTotals']);db.execute('UPDATE state SET payload=?,version=? WHERE id=1',(json.dumps(updated),version+1));db.execute('DELETE FROM previews WHERE token=?',(p['token'],))
