...
class HttpTests(unittest.TestCase):
    ...
    def test_teaching_cost_read_only_and_permission(self):
        ...
        self.assertEqual(data['status'],'PROJECAO_2027_CONCILIADA')
        self.assertEqual(data['validated_cost_cents'],2726065)
    ...
    def test_integrated_financial_and_pe_endpoints_read_only(self):
        ...
        for path in ('/api/financial/teaching-integration?year=2027','/api/break-even/integrated?year=2027'):
            ...
            data=json.loads(body);self.assertEqual(data['teachingCostWeekly'],27260.65)
            self.assertEqual(data['teachingCostMonthly'],122673.00);self.assertEqual(data['monthlyFactor'],4.5);self.assertIsNone(data['breakEvenStudents'])
            ...
            self.assertEqual(g2['breakEvenStudents'],16);self.assertEqual(g2['physicalMarginStudents'],2)
            self.assertEqual(g2['otherDirectCostsMonthly'],750);self.assertEqual(g2['indirectExpensesMonthly'],10395.49);self.assertEqual(g2['totalCostMonthly'],13543.09)
            g3=next(row for row in data['classes'] if row['name']=='G3 A')
            self.assertEqual(g3['teachingCostWeekly'],518.40);self.assertEqual(g3['teachingCostMonthly'],2332.80)
            third=next(row for row in data['classes'] if row['name']=='3º A')
            self.assertEqual(third['teachingCostWeekly'],502.20);self.assertEqual(third['teachingCostMonthly'],2259.90)
        self.assertEqual(self.server.store.state(),before)
