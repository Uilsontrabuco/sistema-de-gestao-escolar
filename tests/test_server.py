...
class ServerTests(unittest.TestCase):
    ...
    def test_financial_discount_duplicate_invalid_cancel_and_recalculation_chain(self):
        ...
        state['breakEven']={'plans':[{'id':'fixture-pe','year':2027,'officialBudget':{'classRows':source,'totals':{'totalExpensesMonthly':205000}},'mappings':mappings,'delinquency':{'scenarioPercent':0},'structuralTicket':{'discountPercent':0,'origin':'fixture'}}]};ledger={'target_year':2027,'conflicted_cost_cents':0,'unassigned_cost_cents':0,'validated_cost_cents':4100,'classes':[{'class_id':room['id'],'weekly_cost':1} for room in state['classes']]}
        ...
        self.assertLess(after_g2['netRevenueMonthly'],before_g2['netRevenueMonthly']);self.assertLess(after_g2['netTicketMonthly'],before_g2['netTicketMonthly']);self.assertEqual(after_g2['breakEvenStudents'],before_g2['breakEvenStudents']);self.assertLess(after['recurringNetRevenueMonthly'],before['recurringNetRevenueMonthly']);self.assertGreaterEqual(after['breakEvenStudents'],before['breakEvenStudents'])
    ...
    def test_real_class_update_pdf_is_read_only_exact_and_replaces_not_adds(self):
        pdf=Path(r'D:\Turmas CAJ 2027 (1).pdf')
        if not pdf.exists():self.skipTest('PDF oficial não disponível neste ambiente')
        state,_=self.store.state();before=copy.deepcopy(state);preview=class_update_preview(pdf.name,pdf.read_bytes(),state)
        self.assertEqual(state,before);self.assertTrue(preview['canConfirm']);self.assertEqual(len(preview['rows']),41);self.assertEqual(preview['newTotals'],{'capacity':1103,'students':752,'new':34,'renewed':718,'vacancies':351})
        expected={'G2 A':(18,12,12,0,6),'G3 A':(24,18,3,15,6),'G4 A':(25,31,4,27,-6),'G5 A':(21,23,2,21,-2),'4º B':(15,22,1,21,-7),'7º A':(30,46,0,46,-16),'1º EM':(48,48,0,48,0),'3º EM':(35,34,0,34,1)}
        for name,values in expected.items():
            row=next(item for item in preview['rows'] if item['className']==name);self.assertEqual((row['newCapacity'],row['newTotal'],row['newNew'],row['newRenewed'],row['newVacancies']),values)
        updated=apply_class_update_preview(state,preview,self.admin);self.assertEqual(len(updated['classes']),41);self.assertEqual(sum(x['students'] for x in updated['classes']),752);self.assertEqual(sum(x['opening']['new'] for x in updated['classes']),34);self.assertEqual(sum(x['opening']['re'] for x in updated['classes']),718);self.assertEqual(next(x for x in updated['classes'] if x['name']=='G2 A')['students'],12);self.assertEqual(state,before)
        duplicate=class_update_preview(pdf.name,pdf.read_bytes(),updated);self.assertTrue(duplicate['duplicateFile']);self.assertFalse(duplicate['canConfirm'])
